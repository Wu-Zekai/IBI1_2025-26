...
for t in range(time_steps):
    ...
    # use np.random.choice to determine how many new infections occur among the susceptible individuals
    # The probability that each susceptible individual gets infected is p_inf, and the probability of not getting infected is 1 - p_inf, so we sample S_count susceptible individuals
    # Note: If S_count is 0, choice will raise an error, so we add a check
    if S_count > 0:
    ...
    # The probability that each infected individual recovers = gamma
    p_rec = gamma
    
    # Only calculate recoveries if there are currently infected individuals
    if I_count > 1: # The problem states that initially 1 person is infected, so we sample from these I individuals
